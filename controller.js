var controller = (function() {
    ...
    //---------------- Variables ----------------
    var myView = null;
    // XHR objects to issue calls to Trimet Web API
    var arrivalsXHR = new XMLHttpRequest();
    var tripXHR = new XMLHttpRequest();

    //-------------- Private Methods -------------
    arrivalsXHR.onreadystatechange = function () {
        if (arrivalsXHR.readyState === RESPONSE_READY) {
            if( arrivalsXHR.status === HTTP_OK) {
                var jsonObj = JSON.parse(arrivalsXHR.responseText);
                myView.updateWithJSONData(jsonObj);
            }
        }
    }
    tripXHR.onreadystatechange = function () {
        if (tripXHR.readyState === RESPONSE_READY) {
            if( tripXHR.status === HTTP_OK) {
                myView.updateWithXMLData(tripXHR.responseXML);
            }
        }
    }
    //-------------- Public Methods -------------
    ...
    function requestTripData(time, from, to) {
        ...
        fullTripUrl = TRIP_URL +
            ...
            "/appId/" + APP_ID;
        tripXHR.open("GET", fullTripUrl,true);
        tripXHR.send();
    }
    function requestArrivalsData(stopIDString) {
        ...
        // Make request
        arrivalsXHR.open("GET", fullArrivalsURL);
        arrivalsXHR.send();
    }
